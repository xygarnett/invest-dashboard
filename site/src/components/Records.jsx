  ...
  return (
    <div className="card">
      <div className="card-head">
        ...
        <div className="card-hint">数量与现价随持仓自动同步</div>
      </div>
...
export function PriceHistory({ data }) {
  ...
  return (
    <div className="card">
      <div className="card-head">
        <div className="card-title">个股行情历史</div>
...
/* ---------- 交易记录 ---------- */
export function Trades({ data, compact = false, defaultRows = 5 }) {
  const all = data.trades || [];
  const [showAll, setShowAll] = useState(false);
  const [openNote, setOpenNote] = useState(null);
  const rows = compact || showAll ? all : all.slice(0, defaultRows);
  const latest = all.length ? all[0].date : '—';
  if (!all.length) return <div className="empty"><strong>暂无数据</strong></div>;
  return (
    <div>
      <div className="tbl-wrap">
        <table className="tbl tbl-trades">
          ...
          <tbody>
            {rows.map((t, i) => {
              ...
              const shown = !long || openNote === i ? note : note.slice(0, 34) + '…';
              return (
                <tr key={t.date + t.code + i}>
                  ...
                  <td className="cell-left note-cell">
                    <span className={openNote === i ? 'note-open' : 'note-closed'}>{shown}</span>
                    {long ? (
...
export function BrokerYearly({ data }) {
  ...
  if (!years.length) return <div className="empty"><strong>暂无数据</strong></div>;
        ...
        return (
          <div className="year-row" key={y}>
            ...
            <span className="year-bar">
              <span className="fill" style={{ width: w.toFixed(1) + '%', background: v >= 0 ? 'var(--up)' : 'var(--down)' }} />
            </span>
